...
Версия 0.4.0.1 включает state machine, визуальные журналы P0/P1/P2, несколько
...
`CATCHING UP` и возврат replica в ISR после синхронизации.

Блок Network & Retry Lab моделирует потерю request до Broker и потерю ACK уже
после append. Сравнение показывает количество попыток Producer, реальных
records и offsets, возникновение duplicate без idempotence и подавление retry
по `producerId + sequence`. Для управления state machine добавлены шаг назад,
повтор анимации, горячие клавиши и быстрый переход между двумя вариантами.
